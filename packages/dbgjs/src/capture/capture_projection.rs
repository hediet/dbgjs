...
fn matches_source_path(location: &SourceLocation, prefix: &str) -> bool {
    crate::capture::coverage::coverage_filter::normalize_path(&location.source_url)
        .starts_with(&crate::capture::coverage::coverage_filter::normalize_path(prefix))
}
        ...
        for function in &mut source.functions {
            if function.effective_ranges.is_empty() {
                function.effective_ranges =
                    crate::debugger::target_debugger::effective_coverage_ranges(&function.ranges);
            }
        }
...
) -> Result<(), crate::debugger::target_debugger::TargetDebuggerError> {
...
pub(crate) fn project_stored_cpu_with_sources(
    ...
    crate::debugger::target_debugger::aggregate_cpu_profile(snapshot)
}
...
mod tests {
    ...
    fn raw_coverage_supports_two_views_without_changing_payload() {
        ...
        crate::capture::coverage::coverage_filter::filter_coverage(&mut selected, None, Some("**/a.ts")).unwrap();
    ...
    fn exclusion_applies_to_raw_cumulative_ranges_before_projection() {
        ...
        let mut delta = crate::debugger::target_debugger::exclude_coverage(selected, &baseline);
