...
use crate::connection::providers::{ConnectionRuntime, validate_configuration};
use crate::service::context_engine::{
...
use crate::debugger::debugger_engine::{SessionKey, StepKind};
use crate::connection::discovery::{DiscoveryState, PauseChildrenLease};
use crate::debugger::resource_graph::{
...
use crate::source::source_graph::{IdentityBasis, ProjectionKind, SourceRevision};
use crate::source::source_search::{
    HydratedSource, SearchControl, SearchDocument, SearchError, SearchQuery, SourceIdentity,
};
use crate::source::source_view::appears_minified;
use crate::debugger::target_debugger::{
    TargetBreakpointSpec, TargetDebuggerError, TargetDebuggerHandle, stored_heap_classes,
};
use crate::debugger::target_domain::target_snapshot_from_info;
...
impl DebuggerService {
    ...
    async fn supervise_target_events(
        ...
        tokio::spawn(async move {
            ...
            while let Some(event) = events.recv().await {
                if let Ok(crate::debugger::cdp_runtime::RootCdpEvent::TargetDetached {
                    session_id,
                    target_id: _,
                }) = &event {
                    let key = {
                ...
                let update = match event {
                    Ok(crate::debugger::cdp_runtime::RootCdpEvent::TargetCreated(params)) => {
                        ConnectionTargetUpdate::Upsert(canonicalize_synthetic_target(
                            target_snapshot_from_info(params.target_info),
                            &connection_id,
                        ))
                    }
                    Ok(crate::debugger::cdp_runtime::RootCdpEvent::TargetChanged(params)) => {
                        ConnectionTargetUpdate::Upsert(canonicalize_synthetic_target(
                            target_snapshot_from_info(params.target_info),
                            &connection_id,
                        ))
                    }
                    Ok(crate::debugger::cdp_runtime::RootCdpEvent::TargetDestroyed(params)) => {
                        ConnectionTargetUpdate::Remove(canonicalize_synthetic_target_id(
                            &params.target_id,
                            &connection_id,
                        ))
                    }
                    Ok(crate::debugger::cdp_runtime::RootCdpEvent::TargetDetached { .. }) => continue,
                    Err(error) => {
                ...
                if transition.change == crate::service::context_engine::ContextChange::None {
                    continue;
                }
        ...
        tokio::spawn(async move {
            ...
            while let Some(event) = events.recv().await {
                let (update, target_to_attach) = match event {
                    crate::connection::providers::ProviderTargetEvent::Upsert(target) => {
                        let target = canonicalize_synthetic_target(target, &connection_id);
                        let target_id = target.target_id.clone();
                        (ConnectionTargetUpdate::Upsert(target), Some(target_id))
                    }
                    crate::connection::providers::ProviderTargetEvent::Removed(target_id) => (
                        ConnectionTargetUpdate::Remove(canonicalize_synthetic_target_id(
                            &target_id,
                            &connection_id,
                        )),
    ...
    fn commit_context(
        ...
        state: &mut ServiceState,
        context_id: &str,
        transition: crate::service::context_engine::ContextTransition,
    ) -> ContextSnapshot {
...
fn context_event_snapshot(event: &crate::service::context_engine::RevisionEvent) -> ContextEventSnapshot {
    use crate::service::context_engine::ContextEvent;
...
struct StoredCapture {
    ...
    heap_mapping: Option<crate::api::service_api::HeapMappingSnapshot>,
}
...
fn select_stored_capture<'a>(
    ...
    let resolved_selector = canonical_target.as_ref().map(|target| {
        crate::debugger::target_selector::resolved_target_selector(
    ...
    if let Some(index) =
        crate::api::service_api::capture_relative_index(selector).map_err(invalid_params)?
    {
...
impl DebuggerService {
        ...
        let reservation = {
            ...
            if reservation.metadata.connection_id != connection_id
                || (reservation.metadata.target_id != target_id
                    && crate::debugger::target_selector::qualified_target_selector(
                        connection_id,
    ...
    async fn reserve_capture_optional(
        ...
        let name = match name {
            Some(name) => {
                ...
                if crate::api::service_api::capture_relative_index(&name)
    ...
    async fn attach_target_internal(
        ...
        let intent_lock = self.breakpoint_intent_lock(&context_id).await;
        ...
        if let Err(error) = Self::seed_attached_breakpoints(&debugger, context_revision, breakpoints).await {
            let mut state = self.state.lock().await;
            if state
                .target_debuggers
                .get(&debugger_key)
    ...
    async fn seed_attached_breakpoints(
        debugger: &TargetDebuggerHandle,
            ...
            .ok_or_else(|| {
                ...
                if generation.is_some_and(|generation| {
                    ...
                    let reason = state.runtimes
                        ...
                        .unwrap_or_default();
                    let selector = crate::debugger::target_selector::qualified_target_selector(
    ...
    pub(crate) async fn relay_target_observation(
        ...
        (
            ...
            watch::Receiver<crate::debugger::resource_graph::GraphRevision>,
        ),
        JsonRpcError,
    > {
        let mut state = self.state.lock().await;
    ...
    pub(crate) async fn relay_open_cdp_session(
        ...
        connection_id: &str,
        session_id: &str,
    ) -> Result<crate::debugger::cdp_runtime::CdpDebuggerSession, JsonRpcError> {
    ...
    async fn register_relay(
        ...
        let crate::connection::relay::context_relay::RelaySession {
            websocket_url,
            cancel,
            completion,
        } = relay;
    ...
    fn resolve_canonical_target(
        ...
        let matches = crate::debugger::target_selector::select_target_matches(
            &candidates,
            context
                .connections
                .iter()
            ...
            Some(selector),
            |target| crate::debugger::target_selector::TargetSelectorCandidate {
        ...
        match matches.as_slice() {
            ...
            _ => Err(invalid_params(&format!(
                ...
                matches
                    .iter()
                    .map(
                        |target_id| crate::debugger::target_selector::qualified_target_selector(
                            connection_id,
                            target_id,
                            connection.generation,
                        )
...
fn sync_connection_resource_graph(
    ...
    let root_kind = match connection.configuration {
        ...
        ConnectionConfiguration::Stdio { topology, .. } => match topology {
            crate::api::service_api::CdpStdioTopology::Browser => "browser",
            crate::api::service_api::CdpStdioTopology::Target => "runtime",
        },
    };
...
fn connection_targets_from_graph(
    snapshot: &crate::debugger::resource_graph::GraphSnapshot,
    connection_id: &str,
    generation: u64,
...
fn debugged_resources(snapshot: &crate::debugger::resource_graph::GraphSnapshot) -> BTreeSet<ResourceId> {
    snapshot
        .relations
        .iter()
...
fn snapshot(
    ...
    id: &str,
    context: &ContextState,
    graph: Option<&crate::debugger::resource_graph::GraphSnapshot>,
) -> ContextSnapshot {
    ...
    for breakpoint in &mut result.breakpoints {
        ...
        let installed = applications
            ...
            .filter(|application| {
                matches!(
                    application.status,
                    crate::api::service_api::TargetBreakpointStatus::Installed { .. }
                )
            })
            .count() as u32;
        ...
        breakpoint.status = if applications.is_empty() {
            ...
            .find_map(|application| match &application.status {
                crate::api::service_api::TargetBreakpointStatus::Failed { message } => {
                    Some(message.clone())
                }
                _ => None,
            })
...
fn breakpoint_pending_reason(
    applications: &[crate::api::service_api::TargetBreakpointSnapshot],
) -> BreakpointPendingReason {
    use crate::api::service_api::TargetBreakpointStatus;
...
fn migrate_capture_order(stored: &mut StoredServiceState) {
    ...
    for (index, capture) in stored.captures.iter_mut().enumerate() {
        ...
        if crate::api::service_api::capture_relative_index(&metadata.name)
...
fn uncompacted_source_node_snapshot(
    source: crate::source::source_graph::SourceSnapshot,
...
fn content_hash_string(content: crate::capture::content_store::ContentHash) -> String {
    content
        .as_bytes()
        .iter()
        .map(|byte| format!("{byte:02x}"))
...
mod tests {
        ...
        state.target_debuggers.insert(
            ...
            TargetDebuggerHandle::ownership_stub_for_tests(
                TargetDebuggerSnapshot {
                    ...
                    revision: 1,
                    phase: crate::api::service_api::TargetDebuggerPhase::Running,
                    scripts: vec![],
                    breakpoints: vec![],
                    logs: vec![],
    ...
    fn process_projection_contributes_windows_sessions_and_targets_to_the_resource_graph() {
        let process = |process_id, parent_process_id, role, window_id, window_title| {
            crate::api::service_api::ProcessSnapshot {
                process_id,
                parent_process_id,
                attachable: true,
        ...
        let mut root = process(
            100,
            None,
            crate::api::service_api::ProcessRole::VscodeMain,
            None,
            None,
        );
        root.agent_sessions
            .push(crate::api::service_api::AgentSessionSnapshot {
        ...
        let tree = ProcessTreeSnapshot {
            root_process_id: 100,
            root_kind: crate::api::service_api::ProcessRootKind::Vscode,
            processes: vec![
                root,
                process(
                    200,
                    Some(100),
                    crate::api::service_api::ProcessRole::Renderer,
                    Some(7),
                    Some("Project".to_owned()),
                ),
            ],
            runtime_metadata_available: true,
            targets: vec![crate::api::service_api::ProcessTargetSnapshot {
                process_id: Some(200),
                attachment: None,
                target: TargetSnapshot {
        ...
        let overlapping_tree = ProcessTreeSnapshot {
            root_process_id: 200,
            root_kind: crate::api::service_api::ProcessRootKind::Node,
            processes: vec![process(
                200,
                None,
                crate::api::service_api::ProcessRole::Node,
                None,
                None,
            )],
    ...
    fn insert_context_with_targets(
        ...
        graph
            .try_update(|graph| {
                ...
                Ok::<_, crate::debugger::resource_graph::GraphError>(())
            })
            .unwrap();
    ...
    async fn target_claim_queued_with_context_update_never_persists_rejected_intent() {
        ...
        let lock = service.breakpoint_intent_lock("ctx").await;
    ...
    #[tokio::test]
    async fn attachment_bootstrap_seeds_context_owner_before_target_claim() {
        use std::future::{Future, poll_fn};
        use std::task::Poll;
        ...
        let previous = state.contexts["ctx"].clone();
        let context = reduce_context(&previous, ContextInput::UserCommand(UserCommand::PutBreakpoint {
            breakpoint_id: "log:shared".into(),
            source_path: "file:///prior.js".into(),
            line: 2,
            column: 1,
        ...
        let target_owned = Arc::new(AtomicBool::new(false));
        let context_owned = Arc::new(AtomicBool::new(false));
        let debugger = TargetDebuggerHandle::ownership_stub_for_tests(
            TargetDebuggerSnapshot {
                context_id: "ctx".into(),
                connection_id: "runtime".into(),
        ...
        let service = service_with_state(PathBuf::from("unused"), state);
        let intent = service.breakpoint_intent_lock("ctx").await;
        let guard = intent.lock().await;
        service.state.lock().await.target_debuggers.insert(
            ("ctx".into(), "runtime".into(), "target-a".into()),
            debugger.clone(),
        );
        let call = CallCtx::default();
        let mut claim = Box::pin(service.set_logpoints(
            &call,
            TargetRef {
                connection: ConnectionRef { context_id: "ctx".into(), connection_id: "runtime".into() },
                target_id: "target-a".into(),
            ...
            vec![LogpointSpec {
                id: "shared".into(),
                source_url: "file:///prior.js".into(),
                line: 2,
                column: 1,
                expression: "1".into(),
            }],
        ));
        assert!(poll_fn(|cx| Poll::Ready(claim.as_mut().poll(cx))).await.is_pending());
        let breakpoint = TargetBreakpointSpec {
            id: "log:shared".into(),
            source_url: "file:///prior.js".into(),
            line: 2,
            column: 1,
        ...
        DebuggerService::seed_attached_breakpoints(
            &debugger, context.revision, vec![breakpoint],
        ).await.unwrap();
        assert!(context_owned.load(Ordering::SeqCst));
        assert!(!target_owned.load(Ordering::SeqCst));
        drop(guard);
        let error = claim.await.unwrap_err();
        assert!(format!("{error:?}").contains("belongs to the context"), "{error:?}");
        assert!(!target_owned.load(Ordering::SeqCst));
        let state = service.state.lock().await;
        assert_eq!(state.contexts["ctx"].revision, context.revision);
        assert_eq!(state.contexts["ctx"].breakpoints["log:shared"], context.breakpoints["log:shared"]);
        assert!(state.target_debuggers.contains_key(&("ctx".into(), "runtime".into(), "target-a".into())));
    }
    ...
    fn relative_capture_selector_grammar_has_one_positive_index_form() {
        use crate::api::service_api::capture_relative_index;
    ...
    fn exclusion_test_coverage(
        ...
        if enriched {
            ...
            for range in &mut function.ranges {
                range.authored_start = Some(crate::api::service_api::SourceLocation {
                    source_url: "src/app.ts".into(),
                    line: range.start_offset + 1,
                    column: 1,
                });
                range.authored_end = Some(crate::api::service_api::SourceLocation {
                    source_url: "src/app.ts".into(),
    ...
    fn coverage_exclusion_matches_runtime_identity_and_ignores_zero_baseline_counts() {
        ...
        assert_eq!(
            crate::debugger::target_debugger::exclude_coverage(selected.clone(), &zero_baseline),
            selected,
        );
        for different_identity in ["script", "url", "name", "mode", "root"] {
            ...
            assert_eq!(
                crate::debugger::target_debugger::exclude_coverage(selected.clone(), &baseline),
                selected,
                "{different_identity}",
            );
        }
        ...
        let excluded = crate::debugger::target_debugger::exclude_coverage(selected, &baseline);
    ...
    async fn stored_coverage_path_filters_normalized_directory_descendants() {
        ...
        let source =
            |script_id: &str, generated_url: &str, associated_authored_source: Option<&str>| {
                crate::api::service_api::CoverageSourceSnapshot {
    ...
    async fn stored_cpu_profile_rebuilds_analysis_without_live_target() {
        ...
        let snapshot = CpuProfileSnapshot {
            ...
            start_time_micros: 1.0,
            end_time_micros: 2.0,
            nodes: vec![crate::api::service_api::CpuProfileNodeSnapshot {
                id: 1,
                call_frame: crate::api::service_api::CpuProfileCallFrameSnapshot {
    ...
    fn stored_heap_maps_survive_service_restart_without_a_live_target() {
        ...
        runtime.block_on(async {
            ...
            let mut supply = crate::api::service_api::HeapSourceMapSupply {
    ...
    fn target_selector_resolution_preserves_nested_connection_prefixes() {
        ...
        for selector in ["browser/browser/frame", "browser/browser/frame@7"] {
            ...
            let request = crate::debugger::target_selector::resolved_target_selector(
    ...
    fn qualified_target_selectors_round_trip_nested_ids_and_generations() {
        ...
        for selector in [
            ...
            crate::debugger::target_selector::qualified_target_selector("process-tree-2", target_id, 7),
        ] {
            let resolved =
                DebuggerService::resolve_canonical_target(&state, "test", &selector).unwrap();
    ...
    async fn qualified_target_requests_reject_reconnect_after_resolution() {
        ...
        let request_target = crate::debugger::target_selector::resolved_target_selector(
            &resolved.connection_id,
            &resolved.target_id,
            resolved.connection_generation,
            Some(selector),
    ...
    async fn attach_reuses_existing_renderer_debugger_before_graph_projection_catches_up() {
        ...
        let runtime = crate::connection::providers::raw_session_tests::runtime(3).await;
    ...
    fn source_view_selection_keeps_exactly_one_unmapped_representation() {
        let content = |path: &str, kind: &str, text: &str| HydratedSource {
            ...
            content_hash: crate::capture::content_store::ContentHash::of_bytes(text.as_bytes()),
