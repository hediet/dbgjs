...
impl DebuggerService {
    async fn stored_heap_capture(
        ...
        target: &TargetRef,
        capture_id: &str,
    ) -> Result<Option<(String, Option<crate::api::service_api::HeapMappingSnapshot>, String)>, HeapProfilerError> {
        let stored = {
    ...
    async fn with_stored_heap_graph<T: Send + 'static>(
        ...
        capture_id: &str,
        enrich_source: bool,
        analyze: impl FnOnce(crate::debugger::target_debugger::StoredHeapGraph)
        ...
        let (prepared, diagnostics) = if enrich_source {
            crate::debugger::target_debugger::prepare_heap_view_sources(mapping.as_ref()).await
        } else {
            (Default::default(), Vec::new())
        };
        tokio::task::spawn_blocking(move || {
            let mapping = if enrich_source {
                crate::debugger::target_debugger::recover_heap_mapping_for_view(mapping, &prepared, &diagnostics)
            } else {
                mapping
            };
            let graph = crate::debugger::target_debugger::StoredHeapGraph::open(
...
impl HeapProfilerApi for DebuggerService {
    ...
    async fn diff_heap_snapshots(
            ...
            self.stored_heap_capture(&target_ref, &older_capture_id).await?
        {
            ...
            return tokio::task::spawn_blocking(move || {
                let older = crate::debugger::target_debugger::StoredHeapGraph::open(
                    Path::new(&older_path), older_name, older_mapping)?;
                let newer = crate::debugger::target_debugger::StoredHeapGraph::open(
