...
impl RelayApi for DebuggerService {
    async fn open_playwright_proxy(
        ...
        let relay = crate::connection::relay::context_relay::start_connection_relay(
            self.clone(),
            context_id.clone(),
            connection_id.clone(),
            format!("{id}-relay"),
        ...
        let proxy = match crate::connection::providers::playwright_proxy::start(
            crate::connection::providers::playwright_proxy::PlaywrightCdpSource::BrowserRoot {
                endpoint: relay.websocket_url.clone(),
            },
            crate::connection::providers::playwright_proxy::PlaywrightPageScope {
        ...
        let crate::connection::providers::playwright_proxy::PlaywrightProxy {
            websocket_url,
            cancel,
            completion,
        } = proxy;
    ...
    async fn open_context_relay(
        ...
        let id = random_instance_id().map_err(|error| internal_error(error.to_string()))?;
        let relay =
            crate::connection::relay::context_relay::start_context_relay(self.clone(), context_id.clone(), id.clone())
                .await
                .map_err(|error| internal_error(error.to_string()))?;
    ...
    async fn open_target_relay(
        ...
        let id = random_instance_id().map_err(|error| internal_error(error.to_string()))?;
        let relay = crate::connection::relay::context_relay::start_target_relay(
            self.clone(),
            context_id.clone(),
            connection_id,
            target_id,
