...
impl TargetDebuggerApi for DebuggerService {
    ...
    async fn get_logs(
        ...
        _ctx: &CallCtx,
        target_ref: TargetRef,
    ) -> Result<crate::api::service_api::TargetLogSnapshot, TargetError> {
        let TargetRef {
            connection:
                ConnectionRef {
                    context_id,
        ...
        use crate::api::service_api::{LogCaptureSnapshot, LogCaptureStatus, TargetLogSnapshot};
        let state = self.state.lock().await;
...
fn logpoint_condition(id: &str, expression: &str) -> Result<String, JsonRpcError> {
    ...
    let binding = serde_json::to_string(crate::debugger::debugger_driver::LOGPOINT_BINDING_NAME)
