...
pub(super) fn target_selector_rpc_error(
    error: crate::target_selector::TargetSelectorError,
) -> JsonRpcError {
    match error {
        crate::target_selector::TargetSelectorError::NotFound { selector } => {
            TargetError::TargetSelectorNotFound { selector }
        }
        crate::target_selector::TargetSelectorError::StaleGeneration {
            selector,
            connection_id,
            generation,
            current_generation,
...
fn unexpected_target_error(error: TargetDebuggerError) -> RpcCallError {
    match error {
        TargetDebuggerError::Driver(crate::debugger_driver::DebuggerDriverError::Runtime(
            crate::cdp_runtime::CdpRuntimeError::Protocol {
                code,
                message,
                data,
            },
        ...
        TargetDebuggerError::Driver(crate::debugger_driver::DebuggerDriverError::Runtime(
            crate::cdp_runtime::CdpRuntimeError::Call(error),
        )) => error,
...
mod tests {
    ...
    fn upstream_cdp_errors_keep_code_message_and_data() {
        ...
        let error = TargetError::from(TargetDebuggerError::Driver(
            crate::debugger_driver::DebuggerDriverError::Runtime(
                crate::cdp_runtime::CdpRuntimeError::Protocol {
                    code: wire.code,
