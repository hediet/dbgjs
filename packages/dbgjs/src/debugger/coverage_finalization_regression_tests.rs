...
use crate::debugger::debugger_engine::{CapturedScriptSource, ScriptState};
...
async fn coverage_driver_with_url(
    ...
    .unwrap();
    let mut state =
        (*crate::debugger::debugger_engine::reduce(&Arc::new(DebuggerState::default()), Input::Connected)
            .state)
            .clone();
    if mapped {
        let source_map = json!({
...
async fn coverage_capture_delegates_effective_ranges_to_view() {
    ...
    crate::capture::capture_projection::project_stored_coverage(&mut viewed);
