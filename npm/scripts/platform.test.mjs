...
import { platforms, platformKey } from "../dbgjs/lib/platform.mjs";

test("the entry package exposes the dbgjs CLI and TUI launchers", async () => {
	const manifest = JSON.parse(await readFile(new URL("../dbgjs/package.json", import.meta.url)));
	...
	for (const [binary, path] of Object.entries(manifest.bin)) {
		const launcher = await readFile(new URL(`../dbgjs/${path}`, import.meta.url), "utf8");
