...
The current CDP pipeline has two overlapping LinkRPC generation models:

1. Normal application services define a Rust trait annotated with
   `#[link_rpc_interface]`. The macro rewrites that trait and generates the
   provider adapter, typed client, and interface descriptor.
2. CDP JSON is imported into a `LinkRpcInterfaceSchema`, then the standalone
...
- LinkRPC client behavior is generated in two places.
...
```text
CDP protocol JSON
  -> LinkRPC interface schema
  -> standard Rust trait/type generator
  -> standard LinkRPC trait expansion
       -> provider trait
       -> server adapter
       -> typed client
...
- Address CDP commands by their exact flat wire names, such as
  `Debugger.enable`, without adding a LinkRPC interface prefix.
...
The normalized `LinkRpcInterfaceSchema` describes methods, payloads,
...
```rust
#[link_rpc_interface(id = "cdp.protocol")]
pub trait Cdp {
    #[linkrpc(name = "Debugger.enable")]
...
notification. `LinkRpcConnection` and a session `Channel` are callers. Generated
...
CDP needs bare addressing. Ordinary LinkRPC services need interface-qualified or
...
- a standard LinkRPC bidirectional/callback-interface feature.

Whichever representation is selected must be part of the general LinkRPC model,
not an `x-linkrpc-codegen.kind = "serverNotification"` branch that only the
...
3. **Expand trait contract**: use the standard LinkRPC macro/backend to produce a
...
```text
normal LinkRPC consumer
  = generated client + LinkRpcConnection + interface-qualified addressing

mounted LinkRPC consumer
  = generated client + LinkRpcConnection + service-mounted addressing
...
The standard LinkRPC API should remain conceptually:
...
- [ ] Remove CDP-only `x-linkrpc-codegen` directives that have standard trait
...
- [ ] Prove ordinary LinkRPC services retain current behavior.

### Phase 3: generate the CDP trait contract

- [ ] Render CDP types and traits with the standard generator.
- [ ] Expand them through the standard LinkRPC path.
